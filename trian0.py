...
def eval_epoch(eval_loader, model, loss_fn, device, best, model_name, loss_name):
    ...
    if best > val_loss/(batch_idx+1):
        ...
        if torch.cuda.device_count() > 1:
            torch.save(model.module.state_dict(), 'D:\\University\\training-\\model\\{}_{}_{:.4f}.pth'.format(model_name, loss_name, best))
        else:
            torch.save(model.state_dict(), 'D:\\University\\training-\\model\\{}_{}_{:.4f}.pth'.format(model_name, loss_name, best))
    return best 
...
if __name__ == '__main__':
    ...
    data_path = 'C:\\Users\\Reza\\Documents\\dataSets\\handi'
    ...
    train_dataset2 = SingleData(class_name, train_img_list, train_transform2)
    z=train_dataset+train_dataset2
    eval_dataset = SingleData(class_name, eval_img_list, eval_transform)
    train_dataloader = DataLoader(z, batch_size=batch_size, shuffle=True, drop_last=True)
